...
- Per attachment: link, size, date, uploader name
- Annotate button (pencil SVG) — for annotatable extensions (PDF, images)
- Admin: rename button (pencil icon) → toggles inline form with current name pre-filled; Save/Cancel
- Admin: delete button (×) with confirm dialog
- `toggleAttachRename(storedName)` JS function shows/hides the rename form
...
- Parties list: add/remove via AJAX
